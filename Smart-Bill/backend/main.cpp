// Smart-Bill / INVOZA
// Cross-platform C++17 HTTP backend
//
// Windows:
...
string bodyValue(
    ...
    while (
        ...
        body[e] != '}' &&
        !isspace(
            static_cast<unsigned char>(body[e])
        )
    ) {
        e++;
    }
...
void ensureDB() {
    ...
    if (!fs::exists(
        ...
        f <<
            "1|Admin|admin@invoza.com|admin123|"
          << nowISO()
          << "\n";
    }
    ...
    if (!fs::exists(
        ...
        f <<
            "id|name|category|price|stock|sku|supplier|updated_at\n";

        f <<
            "P1001|Paracetamol 500mg|Medicine|25|100|"
          << "PARA500|ABC Pharma|"
          << nowISO()
          << "\n";

        f <<
            "P1002|Notebook A4|Stationery|80|50|"
          << "NOTE-A4|Local Supplier|"
          << nowISO()
          << "\n";

        f <<
            "P1003|Ball Pen|Stationery|10|200|"
    ...
    if (!fs::exists(
        ...
        f <<
            "id|invoice|customer|items|amount|payment|date\n";
    }

    if (!fs::exists(
        DB_DIR + "/purchases.csv"
    )) {
        ...
        f <<
            "id|product|quantity|amount|supplier|date\n";
    }
    ...
    if (!fs::exists(
        ...
        f <<
            "id|invoice|customer|items_json|"
